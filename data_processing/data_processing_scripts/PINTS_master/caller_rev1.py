...
def check_version():
    ...
    try:
        r = requests.get(
            "https://raw.githubusercontent.com/liyao001/PINTS/master/version.txt"
        )
        latest_version = r.text.strip()
        latest_version_lst = list(map(int, latest_version.split(".")))
        version_file = os.path.join(
            os.path.split(os.path.realpath(__file__))[0], "version.txt"
        )
        if not os.path.exists(version_file):
            logger.warning(
                "Cannot locate the version file, please make sure the installation is complete."
            )
            return None
        ...
        for k, sub_ver in enumerate(latest_version_lst):
            if local_version[k] < sub_ver:
                logger.info(
                    "A later version of PINTS is available (%s)" % latest_version
                )
                break
...
def sliding_window(chromosome_coverage, window_size, step_size):
    ...
    total_bins = np.floor(
        chromosome_coverage.shape[0] / step_size - window_size / step_size + 1
    ).astype(int)
    start = 0
    end = window_size
...
def _atom_ler(
    ...
    query_start,
    query_end,
    small_window_threshold=4,
    peak_in_bg_threshold=1,
):
    """
    ...
    try:
        ...
        for sub_peak in bed_handler.fetch(
            chromosome, query_start, query_end, parser=pysam.asTuple()
        ):
            ...
            if (
                peak_dens >= peak_in_bg_threshold
            ):  # and peak_dens < peak_similarity_threshold:
    ...
    if n_candidate > 3:
        ...
        for k, (b, s) in enumerate(uncertain_re_l):
            ...
            background_window = np.concatenate(
                (coverage_info[b - 2500 : b], coverage_info[s : s + 2500]), axis=None
            )
...
def check_window(
    ...
    if coord_end - coord_start < small_window_threshold or window_value == 0:
        return 1.0, window_value, 0, 0, (0, 0, 0)
    ...
    for k, f in enumerate(flanking):
        ...
        qsr = coord_end
        qer = coord_end + f
        bg, x = remove_peaks_in_local_env(
            bed_handler=sp_bed_handler,
            chromosome=chromosome_name,
            query_start_left=qsl,
            query_end_left=qel,
            query_start_right=qsr,
            query_end_right=qer,
            small_window_threshold=small_window_threshold,
            peak_in_bg_threshold=peak_in_bg_threshold,
...
def cut_peaks(
    ...
    widths, cor_heights, starts, ends = peak_widths(
        window, peaks, rel_height=peak_rel_height
    )
    intervals = []
    ...
    for k, c in enumerate(candidates):
        if k < len(candidates) - 1:
            ...
            if new_density >= donor_tolerance * c[3]:
                distance_check = (
                    c[1] - c[0] < ce_trigger
                    or candidates[k + 1][0] - c[1] > max_distance
                )
                if distance_check and new_density < (
                    receptor_tolerance * candidates[k + 1][3]
                ):
                    continue
    ...
    for k in range(len(candidates) - 1, -1, -1):
        ...
        if k > 0:
            ...
            if new_density >= donor_tolerance * c[3]:
                distance_check = (
                    c[1] - c[0] < ce_trigger
                    or c[0] - candidates[k - 1][1] > max_distance
                )
                if distance_check and new_density < (
                    receptor_tolerance * candidates[k - 1][3]
                ):
...
def check_window_chromosome(
    ...
    bins = []
    for window, coord in sliding_window(
        per_base_cov, window_size=window_size, step_size=step_size
    ):
        if window > read_counts_threshold:  # no reads in the bin
            bins.append((chromosome_name, coord[0], coord[1], window))

    logger.info(
        "Before merging, there are %d windows on %s" % (len(bins), chromosome_name)
    )
    ...
    bed_obj = BedTool(
        tmp_df.to_csv(sep="\t", index=False, header=False), from_string=True
    )
    ...
    merged_windows = pd.read_csv(
        merged_fn,
        sep="\t",
        names=["Chromosome", "Start", "End", "Name", "Reads", "Strand"],
    )
    logger.info(
        "After merging, there are %d windows on %s"
        % (merged_windows.shape[0], chromosome_name)
    )
    ...
    for nr, row in merged_windows.iterrows():
        sub_peaks = cut_peaks(
            per_base_cov[row["Start"] : row["End"]],
        ...
        for sp in sub_peaks:
            ...
            n_start_sizes = sum(
                peak_region > 0
            )  # if n start sizes is smaller than 3, then ZIP shouldn't be used
            peak_len = end - start
            if (
                peak_len < small_window_threshold
                or window_value == 0
                or n_start_sizes < 3
            ):
    ...
    log_assert(
        rc == 0 and os.path.exists(subpeak_bed + ".gz"),
    ...
    if len(all_peak_mus) == 0:
        logger.warning(
            "No non-trivial peak was detected on chromosome %s" % chromosome_name
        )
        peak_threshold = 1
        bkg_mu_threshold = 0
    else:
    ...
    with gzip.open(subpeak_bed + ".gz", "rt") as peak_obj:
        for peak in peak_obj:
            ...
            pval, wv, mu_bg, pi_bg, lerc = check_window(
                coord_start=peak_start,
                coord_end=peak_end,
                mu_peak=peak_mu,
                pi_peak=peak_pi,
                chromosome_coverage=per_base_cov,
                peak_in_bg_threshold=peak_threshold,
                mu_bkg_minimum=bkg_mu_threshold,
                sp_bed_handler=bed_handler,
...
def independent_filtering(df, output_to=None, **kwargs):
    ...
    **kwargs :
    ...
    log_assert(
        "reads" in df.columns, "Please provide read counts (reads) in df", logger
    )
    quantiles_tested = []
    windows_remain = []
    windows_rejected = []
    ...
    for quantile in np.arange(0, 1, kwargs["ind_filter_granularity"]):
        ...
        try:
            mult_res = multipletests(
                filtered_df["pval"],
                alpha=kwargs["fdr_target"],
                method=kwargs["adjust_method"],
            )
            read_out = mult_res[1]
        ...
        filtered_df = filtered_df.loc[
            :, ("chromosome", "start", "end", "name", "padj", "strand", "reads")
        ]
        ...
        logger.info(
            "Independent filtering: percentile %f, rejection %d (%d)"
            % (quantile, windows_rejected[-1], df.shape[0])
        )
    ...
    if kwargs["output_diagnostic_plot"] and output_to is not None:
        ...
        ax2.annotate(
            "Read counts: %d" % read_counts_threshold,
            xy=(
                quantiles_tested_arr[np.argmax(windows_rejected_arr)],
                np.max(windows_rejected_arr),
            ),
        )
        ...
        plt.close()
        logger.info(
            "Diagnostic plot for independent filtering was wrote to %s" % output_to
        )
    return final_df


def peaks_single_strand(
    per_base_cov, output_file, window_size, step_size, strand_sign, **kwargs
):
    """
    ...
    **kwargs :
    ...
    for chrom, pbc_npy in per_base_cov.items():
        ...
        args.append(
            (
                ...
                window_size,
                step_size,
                kwargs["read_counts_threshold"],
                kwargs["peak_height"],
                kwargs["peak_threshold"],
                kwargs["peak_distance"],
                kwargs["peak_prominence"],
                kwargs["peak_width"],
                kwargs["peak_wlen"],
                kwargs["peak_rel_height"],
                kwargs["fdr_target"],
                kwargs["donor_tolerance"],
                kwargs["receptor_tolerance"],
                kwargs["ce_trigger"],
            )
        )
    ...
    corrected_small_peaks = np.sum(
        np.logical_and(tmp_df["pval"] < kwargs["fdr_target"], small_peaks_probe)
    )
    logger.info(
        "Significant small peaks after correction: %d (%d)"
        % (corrected_small_peaks, inflated_small_peaks)
    )
    if kwargs["output_diagnostic_plot"]:
        pval_dist(
            tmp_df.loc[
                tmp_df["end"] - tmp_df["start"] > kwargs["small_peak_threshold"], "pval"
            ],
            logger=logger,
        ...
        pval_dist(
            tmp_df.loc[
                tmp_df["end"] - tmp_df["start"] <= kwargs["small_peak_threshold"],
                "pval",
            ],
            logger=logger,
            output_diagnostic_plot=kwargs["output_diagnostic_plot"],
    ...
    tmp_df_sm = independent_filtering(
        tmp_df.loc[small_peaks_probe, :], output_to=fn + "_idpf_sm.pdf", **kwargs
    )
    tmp_df_bg = independent_filtering(
        tmp_df.loc[big_peaks_probe, :], output_to=fn + "_idpf_bg.pdf", **kwargs
    )
...
def merge_opposite_peaks(
    sig_peak_bed,
    peak_candidate_bed,
    divergent_output_bed,
    bidirectional_output_bed,
    singleton_bed,
    fdr_target,
    **kwargs
):
    ...
    **kwargs :
    ...
    for nr, line in enumerate(fh):
        ...
        try:
            query_start = query_start if query_start >= 0 else 0
            for hit in tbx.fetch(
                items[0], query_start, query_end, parser=pysam.asTuple()
            ):
                hit_start = int(hit[1])
                hit_end = int(hit[2])
            ...
            logger.warning(
                "Problematic region for %s:%d-%d\n%s"
                % (items[0], query_start, query_end, err)
            )
        if opposite_start is np.nan:
            ...
            if tre_end - tre_start > kwargs["div_size_min"]:
                candidate_values = (
                    ...
                    items[4],
                    items[5],
                    str(float(items[6]) + opposite_sum),
                    items[1],
                    items[2],
                    str(opposite_start),
                    str(opposite_end),
...
def peak_calling(
    input_bam, output_dir=".", output_prefix="pints", filters=[], thread_n=1, **kwargs
):
    """
    ...
    if input_bam == "bigwig":
        log_assert(
            os.path.exists(kwargs["bw_pl"]),
            "Cannot find bigwig file %s" % kwargs["bw_pl"],
            logger,
        )
        log_assert(
            os.path.exists(kwargs["bw_mn"]),
            "Cannot find bigwig file %s" % kwargs["bw_mn"],
            logger,
        )
    else:
        log_assert(
            kwargs["bam_parser"] is not None,
            "Please specify which type of experiment this data "
            "was generated from with --exp-type",
            logger,
        )
        log_assert(
            os.path.exists(input_bam),
            "Cannot find input bam file %s" % input_bam,
            logger,
        )
    log_assert(
        os.path.exists(output_dir) and os.path.isdir(output_dir),
        "Cannot write to %s" % output_dir,
        logger,
    )
    ...
    if input_bam != "bigwig":
        chromosome_coverage_pl, chromosome_coverage_mn = get_read_signal(
            input_bam=input_bam,
        ...
        chromosome_coverage_pl, chromosome_coverage_mn = get_coverage_bw(
            output_dir=output_dir, output_prefix=output_prefix, **kwargs
        )
    ...
    # peak calling (IQR)
    pl_peaks_bed = peaks_single_strand(
        per_base_cov=chromosome_coverage_pl,
    ...
    mn_peaks_bed = peaks_single_strand(
        per_base_cov=chromosome_coverage_mn,
    ...
    pl_df = pd.read_csv(
        prefix + "_pl.bed.gz", sep="\t", header=None, names=COMMON_HEADER
    )
    mn_df = pd.read_csv(
        prefix + "_mn.bed.gz", sep="\t", header=None, names=COMMON_HEADER
    )
    # filter huge merged bins
    pl_df = pl_df.loc[
        pl_df["end"] - pl_df["start"] < kwargs["window_size_threshold"], :
    ]
    mn_df = mn_df.loc[
        mn_df["end"] - mn_df["start"] < kwargs["window_size_threshold"], :
    ]
    ...
    command = (
        "cat %s %s | sort -k1,1 -k2,2n | bedtools merge -i - -c 12 -o distinct > %s"
        % (
            prefix + "_sig_pl_bidirectional_peaks.bed",
            prefix + "_sig_mn_bidirectional_peaks.bed",
    ...
    command = (
        "cat %s %s | sort -k1,1 -k2,2n | bedtools merge -i - -c 12 -o distinct> %s"
        % (
            prefix + "_sig_pl_divergent_peaks.bed",
            prefix + "_sig_mn_divergent_peaks.bed",
            prefix + "_divergent_peaks.bed",
        )
    )
    run_command(command)
    command = "cat %s %s | sort -k1,1 -k2,2n > %s" % (
        prefix + "_sig_pl_singletons.bed",
    ...
    cmd = (
        "cat %s %s | sort -k1,1 -k2,2n | awk 'BEGIN{OFS=\"\\t\"}{print $1,$2,$3,$4,$11,$6,$7,$8,$5,$13-$2}' > %s"
    ...
    logger.info(
        "Bidirectional peaks were saved to %s" % prefix + "_bidirectional_peaks.bed"
    )
    logger.info(
        "Significant peaks which failed to pair were saved to %s" % prefix
        + "_single_peaks.bed"
    )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Peak Identifier for Nascent Transcript Sequencing"
    )
    group = parser.add_argument_group("IO")
    group.add_argument(
        "bam_file",
        help="input bam file, if you want to use bigwig files, please leave this as bigwig",
    )
    ...
    group.add_argument(
        ...
        required=False,
        default=100,
        help="size for sliding windows",
    )
    group.add_argument(
        "--max-window-size",
        action="store",
        dest="window_size_threshold",
        type=int,
        required=False,
        default=2000,
        help="max size of divergent windows",
    )
    group.add_argument(
        "--step-size",
        action="store",
    ...
    group.add_argument(
        ...
        required=False,
        default=0.1,
        help="FDR target for multiple testing",
    )
    group.add_argument(
        "--read-counts-threshold",
        action="store",
        dest="read_counts_threshold",
        type=int,
        required=False,
        default=0,
    ...
    group.add_argument(
        ...
        required=False,
        default="fdr_bh",
        help="method for calculating adjusted p-vals",
    )
    group.add_argument(
        "--small-peak-threshold",
        action="store",
        dest="small_peak_threshold",
        type=int,
        required=False,
        default=5,
    ...
    group.add_argument(
        "--ind-filtering-granularity",
        action="store",
        dest="ind_filter_granularity",
        type=int,
        required=False,
        default=0.005,
    ...
    group.add_argument(
        "--donor-tolerance",
        action="store",
        dest="donor_tolerance",
        type=float,
        required=False,
        default=1.0,
        help="Donor tolerance in best score segments",
    )
    group.add_argument(
        "--receptor-tolerance",
        action="store",
        dest="receptor_tolerance",
        type=float,
        required=False,
        default=0.1,
        help="Receptor tolerance in best score segments",
    )
    group.add_argument(
        "--ce-trigger",
        action="store",
        ...
        required=False,
        default=3,
        help="Trigger for receptor tolerance checking",
    )
    ...
    peak_calling(
        ...
        small_peak_threshold=args.small_peak_threshold,
        chromosome_startswith=args.chromosome_startswith,
        peak_height=args.peak_height,
        peak_threshold=args.peak_threshold,
        peak_distance=args.peak_distance,
        window_size_threshold=args.window_size_threshold,
        peak_prominence=args.peak_prominence,
        peak_width=args.peak_width,
        peak_wlen=args.peak_wlen,
        peak_rel_height=args.peak_rel_height,
        output_chrom_size=args.output_chrom_size,
        output_diagnostic_plot=args.output_diagnostic_plot,
        ind_filter_granularity=args.ind_filter_granularity,
        thread_n=args.thread_n,
        div_size_min=args.div_size_min,
        summit_dist_min=args.summit_dist_min,
        top_peak_threshold=args.top_peak_threshold,
        bw_pl=args.bw_pl,
        bw_mn=args.bw_mn,
        donor_tolerance=args.donor_tolerance,
        receptor_tolerance=args.receptor_tolerance,
        ce_trigger=args.ce_trigger,
        bam_parser=args.bam_parser,
        seq_rc=args.seq_reverse_complement,
