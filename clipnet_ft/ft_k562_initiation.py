...
initiation_files = [
    f"../../data/k562/initiation/k562_initiation_G{i}.bed" for i in (1, 5, 6)
]
names = ("chrom", "start", "stop", "name", "y", "strand")
initiation = pd.concat(
    [pd.read_csv(fp, header=None, sep="\t", names=names) for fp in initiation_files]
)

# Load sequences
twohot = utils.get_twohot_fasta_sequences(
    "../../data/k562/initiation/k562_initiation_jitter.fa.gz"
)
sequences = np.concatenate([twohot for _ in initiation_files])
...
train_idx = initiation.chrom.isin(train_chroms)
val_idx = initiation.chrom.isin(val_chroms)
train_gen = DataGenerator(sequences[train_idx], initiation.y[train_idx].to_numpy())
val_gen = DataGenerator(sequences[val_idx], initiation.y[val_idx].to_numpy())
...
# Create callbacks
outdir = Path(f"../models/clipnet_k562_initiation/f{fold}/")
model_filepath = str(outdir.joinpath("clipnet_k562_initiation.h5"))
...
csv_logger = CSVLogger(
    filename=outdir.joinpath("clipnet_k562_initiation.log"),
    separator=",",
    append=True,
)
