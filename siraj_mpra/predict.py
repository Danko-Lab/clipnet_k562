...
oligos = pd.read_csv("../../siraj_mpra/media-3_oligos_snps.tsv.gz", sep="\t")
variants = pd.read_csv("../../siraj_mpra/media-3_oligos_snps.tsv.gz", sep="\t")[
    "Variant"
]
...
oligos["pred_p"] = np.log2(
    h5py.File("k562_mpra_snps_ref_procapnet.h5")["quantity"][:]
    / h5py.File("k562_mpra_snps_alt_procapnet.h5")["quantity"][:]
)

oligos["pred"] = np.log2(
    h5py.File("k562_mpra_snps_ref.h5")["quantity"][:]
    / h5py.File("k562_mpra_snps_alt.h5")["quantity"][:]
)
