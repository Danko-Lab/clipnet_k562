...
padded_seqs = []
for name, seq in tqdm.tqdm(seqs.items(), total=len(seqs), desc="Padding sequences with Ns"):
    if len(seq) == 196_608:
        ...
        if start == "0":
            ...
            seq = seq + "N" * padding
        padded_seqs.append(seq)
ref = [one_hot_encode(seq) for seq in tqdm.tqdm(padded_seqs)]
...
with h5py.File("/workdir/ayh8/data/k562_siraj_mpra/k562_mpra_snps_ref_enformer_all.h5", "w") as f:
