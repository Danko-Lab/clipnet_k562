...
observed = average_by_modulo(
    np.load("../../data/k562_data_folds/k562_proseq_0.npz")["arr_0"],
    prediction["track"].shape[0],
)

track_corr = pd.DataFrame(observed[:, np.r_[250:750, 1250:1750]]).corrwith(
    pd.DataFrame(prediction["track"]), axis=1
)
pearsonr(
    np.log(observed[:, np.r_[250:750, 1250:1750]].sum(axis=1)),
    np.log(prediction["quantity"][:, 0]),
)
spearmanr(
